...
(function(){
  ...
  let idleReady=false,idlePlayer=null,idleUpgrades={},idleStats=null,idleTickTimer=null,idleRenderTimer=null,idleSaveTimer=null,idleEventTimer=null,idleCasinoBusy=false,idleRebirthBusy=false,idleLastRender=0;
  ...
  function idleSection(){ const v=localStorage.getItem(IDLE_SECTION_KEY)||'spend'; return ['spend','opportunity','casino','goals','rank'].includes(v)?v:'spend'; }
  ...
  async function idleDoRebirth(){ if(!idlePlayer)return; if(idleRebirthBusy){idleToast('인생 2회차 처리 중이야. 잠깐만 기다려줘.','error');return;} idleStats=deriveIdleStats(); const info=idleRebirthInfo(); if(!info.ok){idleToast('아직 인생 2회차 조건이 부족해','error');return;} const ok=confirm(`인생 2회차를 시작할까?
...
획득 포인트: ${info.points}`); if(!ok)return; idleRebirthBusy=true; const btn=$i('idleRebirthBtn'); if(btn){btn.disabled=true;btn.textContent='처리 중...';} try{ const rebirthPlayerId=idlePlayer.id; const beforePrestige=n(idlePlayer.prestige_level); const keep={}; Object.keys(idleUpgrades).forEach(k=>{ if(k.startsWith('legacy_')||k.startsWith('spend_'))keep[k]=idleUpgrades[k]; }); for(const k of Object.keys(idleUpgrades)){ if(!(k.startsWith('legacy_')||k.startsWith('spend_'))){ try{ await sbFetch(`/idle_upgrades?player_id=eq.${encodeURIComponent(rebirthPlayerId)}&upgrade_key=eq.${encodeURIComponent(k)}`,{method:'DELETE',headers:{Prefer:'return=minimal'}}); }catch(e){} } }
  ...
  function idleRenderAll(){ idleStats=deriveIdleStats(); if(!$i('idlePlayerName'))return; const p=idlePlayer; $i('idlePlayerName').textContent=p?p.name:'-'; $i('idleTitleText').textContent=p?idleTitle(p.lifetime_money):'-'; const rt=$i('idleRoundText'); if(rt)rt.textContent=p?`${Number(p.prestige_level||0)+1}회차`:'1회차'; $i('idleMoneyText').textContent=p?won(p.money):'0원'; $i('idleLifeText').textContent=p?won(p.lifetime_money):'0원'; $i('idleCpsText').textContent=p?won(idleStats.income)+'/s':'0원/s'; $i('idleClickText').textContent=p?won(idleStats.click):'0원'; $i('idleOfflineText').textContent=p?`최대 ${idleStats.offlineHours.toFixed(1)}시간`:'-'; const ev=$i('idleEventText'); if(ev){ const until=p&&p.active_event_until?new Date(p.active_event_until).getTime():0; ev.textContent=until>Date.now()?`${eventName(p.active_event_key)} ${Math.ceil((until-Date.now())/1000)}초`:'이벤트 없음'; }
    renderIdleQuickPanel(); renderIdleUpgrades(); renderIdleExtra(); }

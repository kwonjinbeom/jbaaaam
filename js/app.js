...
(function(){
  ...
  function idleRebirthRequirement(){
    ...
    const moneyReq=Math.floor(50000000*Math.pow(7,prestige)*(1+prestige*.35));
    const levelReq=Math.floor(120+prestige*95+prestige*prestige*28);
    const spendReq=prestige<1?0:Math.floor(250+prestige*360+prestige*prestige*90);
  ...
  function idleBlockHeldWorkKey(e){ if(!e.repeat)return; if(e.key==='Enter'||e.key===' '||e.key==='Spacebar'||e.key==='Space'||e.code==='Space'){ e.preventDefault(); e.stopPropagation(); } }
  function idleUpgradeBatchInfo(key,count=1){ const u=UPG[key]; if(!u)return {qty:0,total:0,nextLv:0}; let lv=idleLevel(key),money=n(idlePlayer&&idlePlayer.money),total=0,qty=0; const maxCount=Math.max(1,Number(count)||1); while(qty<maxCount&&lv<u.max){ const cost=idleCost(u,lv); if(money<cost)break; money-=cost; total+=cost; lv++; qty++; } return {qty,total,nextLv:lv}; }
  async function idleBuyUpgrade(key,count=1){ if(!idlePlayer){idleToast('이름을 먼저 불러와줘','error');return;} const u=UPG[key]; if(!u)return; const lv=idleLevel(key); if(lv>=u.max){idleToast('이미 최대 레벨이야');return;} const batch=idleUpgradeBatchInfo(key,count); if(!batch.qty){idleToast(`돈 부족: ${won(idleCost(u,lv))}`,'error');return;} idlePlayer.money=n(idlePlayer.money)-batch.total; idleUpgrades[key]=batch.nextLv; idleStats=deriveIdleStats(); await idleSaveUpgrade(key,batch.nextLv); await idleSavePlayer({money:n(idlePlayer.money),lifetime_money:n(idlePlayer.lifetime_money)}); idleRenderAll(); idleToast(`${u.name} Lv.${batch.nextLv}${batch.qty>1?` · ${batch.qty}개 구매`:''}`); }
  ...
  function idleGoalList(){ const money=n(idlePlayer&&idlePlayer.money), life=n(idlePlayer&&idlePlayer.lifetime_money), clicks=n(idlePlayer&&idlePlayer.total_clicks), cps=idleStats?n(idleStats.income):0, totalLv=idleTotalUpgradeLevel(); return [
    {key:'goal_click_50',name:'첫 출근 50번',desc:'일하기 50회 누르기',ok:clicks>=50,reward:5000},
    {key:'goal_money_50k',name:'첫 5만원 모으기',desc:'총 번 돈 50,000원 달성',ok:life>=50000,reward:18000},
    {key:'goal_upgrade_20',name:'업그레이드 감 잡음',desc:'업그레이드 총합 Lv.20 달성',ok:totalLv>=20,reward:35000},
    {key:'goal_cps_1000',name:'잠자는 돈맛',desc:'초당 수익 1,000원/s 달성',ok:cps>=1000,reward:90000},
    {key:'goal_million',name:'첫 백만장자',desc:'총 번 돈 1,000,000원 달성',ok:life>=1000000,reward:260000},
    {key:'goal_upgrade_120',name:'시스템이 굴러간다',desc:'업그레이드 총합 Lv.120 달성',ok:totalLv>=120,reward:900000},
    {key:'goal_cps_50000',name:'자동수익 머신',desc:'초당 수익 50,000원/s 달성',ok:cps>=50000,reward:4500000},
    {key:'goal_ten_million',name:'인생 폈다',desc:'총 번 돈 10,000,000원 달성',ok:life>=10000000,reward:8000000}
  ]; }
  ...
  function idleDailyReward(){ idleStats=deriveIdleStats(); return Math.max(3000,Math.floor(Math.min(n(idlePlayer&&idlePlayer.lifetime_money)*.018+5000,(idleStats?idleStats.income:0)*900+5000))); }
...
획득 포인트: ${info.points}`); if(!ok)return; idleRebirthBusy=true; const btn=$i('idleRebirthBtn'); if(btn){btn.disabled=true;btn.textContent='처리 중...';} try{ const rebirthPlayerId=idlePlayer.id; const beforePrestige=n(idlePlayer.prestige_level); const keep={}; Object.keys(idleUpgrades).forEach(k=>{ if(k.startsWith('legacy_')||k.startsWith('spend_'))keep[k]=idleUpgrades[k]; }); for(const k of Object.keys(idleUpgrades)){ if(!(k.startsWith('legacy_')||k.startsWith('spend_'))){ try{ await sbFetch(`/idle_upgrades?player_id=eq.${encodeURIComponent(rebirthPlayerId)}&upgrade_key=eq.${encodeURIComponent(k)}`,{method:'DELETE',headers:{Prefer:'return=minimal'}}); }catch(e){} } }
  ...
  function renderIdleUpgrades(){ const list=$i('idleUpgradeList'); if(!list)return; const savedTab=localStorage.getItem(IDLE_TAB_KEY)||'click'; const tab=IDLE_TABS.some(([k])=>k===savedTab)?savedTab:'click'; if(tab!==savedTab)localStorage.setItem(IDLE_TAB_KEY,tab);
    const arr=IDLE_UPGRADES.filter(u=>u.cat===tab); list.innerHTML=arr.map(u=>{ const lv=idleLevel(u.key),cost=idleCost(u,lv),max=lv>=u.max,batch=idleUpgradeBatchInfo(u.key,10),can=idlePlayer&&!max&&n(idlePlayer.money)>=cost,can10=idlePlayer&&!max&&batch.qty>0; return `<div class="idle-upgrade ${can?'can':''}"><div><b>${html(u.name)} <em>Lv.${lv}/${u.max}</em></b><small>${html(u.desc)}</small><small>다음 비용 ${max?'MAX':won(cost)}${!max&&batch.qty?` · x${batch.qty} ${won(batch.total)}`:''}</small></div><div class="idle-upgrade-actions"><button type="button" data-up="${u.key}" ${!can?'disabled':''}>${max?'MAX':'구매'}</button><button type="button" data-up10="${u.key}" ${!can10?'disabled':''}>x10</button></div></div>`; }).join(''); list.querySelectorAll('[data-up]').forEach(b=>b.onclick=()=>idleBuyUpgrade(b.dataset.up,1)); list.querySelectorAll('[data-up10]').forEach(b=>b.onclick=()=>idleBuyUpgrade(b.dataset.up10,10)); renderIdleCasino(); }

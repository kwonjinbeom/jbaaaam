...
(function(){
  ...
  function idleRebirthInfo(){ const life=n(idlePlayer&&idlePlayer.lifetime_money), totalLv=idleTotalUpgradeLevel(), spend=idleSpendBonus(); const ok=life>=50000000&&totalLv>=120; const points=ok?Math.max(1,Math.floor(Math.sqrt(life/10000000))+Math.floor(spend.score/350)):0; return {ok,points,life,totalLv,spend}; }
  ...
  async function idleDoRebirth(){ if(!idlePlayer)return; const info=idleRebirthInfo(); if(!info.ok){idleToast('아직 인생 2회차 조건이 부족해','error');return;} const ok=confirm(`인생 2회차를 시작할까?\n현재 돈/일반 업그레이드/목표 보상 기록은 초기화되고, 돈 쓰기 수집품과 인생 포인트는 유지돼.\n획득 포인트: ${info.points}`); if(!ok)return; const keep={}; Object.keys(idleUpgrades).forEach(k=>{ if(k.startsWith('legacy_')||k.startsWith('spend_'))keep[k]=idleUpgrades[k]; }); for(const k of Object.keys(idleUpgrades)){ if(!(k.startsWith('legacy_')||k.startsWith('spend_'))){ try{ await sbFetch(`/idle_upgrades?player_id=eq.${encodeURIComponent(idlePlayer.id)}&upgrade_key=eq.${encodeURIComponent(k)}`,{method:'DELETE',headers:{Prefer:'return=minimal'}}); }catch(e){} } }
    keep.legacy_points=Number(keep.legacy_points||0)+info.points; idleUpgrades=keep; await idleSaveUpgrade('legacy_points',keep.legacy_points); idlePlayer.money=0; idlePlayer.lifetime_money=0; idlePlayer.total_clicks=0; idlePlayer.prestige_level=n(idlePlayer.prestige_level)+1; idlePlayer.active_event_key=null; idlePlayer.active_event_until=null; await idleSavePlayer({money:0,lifetime_money:0,total_clicks:0,prestige_level:idlePlayer.prestige_level,active_event_key:null,active_event_until:null}); idleRenderAll(); idleToast(`인생 ${idlePlayer.prestige_level+1}회차 시작 · 포인트 +${info.points}`); }
  function renderIdleRebirth(){ const box=$i('idleRebirthBox'); if(!box)return; const info=idleRebirthInfo(); const points=idleLegacyPoints(); box.innerHTML=`<div class="idle-panel idle-rebirth-panel"><h3>인생 2회차</h3><p class="idle-muted">후반에 막히는 구간을 넘기는 환생 시스템이야. 현재 자산과 일반 업그레이드는 리셋되지만, 돈 쓰기 수집품과 인생 포인트 특성은 유지돼.</p><div class="idle-rebirth-status"><span>조건: 총 번 돈 5천만원 + 업그레이드 총합 Lv.120</span><b>${info.ok?'가능':'진행중'}</b><small>현재 ${compact(info.life)}원 / Lv.${info.totalLv} · 예상 포인트 ${info.points}</small></div><button id="idleRebirthBtn" type="button" ${!idlePlayer||!info.ok?'disabled':''}>인생 2회차 시작</button></div><div class="idle-panel"><h3>인생 포인트 특성 <small>보유 ${points}P</small></h3>${idleLegacyPerks().map(p=>{const lv=idleLevel(p.key),can=idlePlayer&&points>=p.cost&&lv<p.max; return `<div class="idle-spend-item ${can?'can':''}"><span><b>${html(p.name)} Lv.${lv}/${p.max}</b><small>${html(p.desc)}</small></span><button type="button" data-legacy="${p.key}" ${!can?'disabled':''}>${lv>=p.max?'MAX':p.cost+'P'}</button></div>`;}).join('')}</div>`; const rb=$i('idleRebirthBtn'); if(rb)rb.onclick=idleDoRebirth; box.querySelectorAll('[data-legacy]').forEach(b=>b.onclick=()=>idleBuyLegacyPerk(b.dataset.legacy)); }
  function renderIdleExtra(){ const box=$i('idleExtraBox'); if(!box)return; const sec=idleSection(); document.querySelectorAll('.idle-section-tab').forEach(b=>b.classList.toggle('active',b.dataset.section===sec)); if(sec==='spend'){box.innerHTML='<div id="idleSpendBox"></div>'; renderIdleSpend();} else if(sec==='opportunity'){box.innerHTML='<div id="idleBuffBox"></div><div id="idleOpportunityBox"></div>'; renderIdleBuffShop(); renderIdleOpportunities();} else if(sec==='casino'){box.innerHTML='<div id="idleCasinoBox"></div>'; renderIdleCasino();} else if(sec==='goals'){box.innerHTML='<div id="idleGoalsBox"></div><div id="idleRebirthBox"></div>'; renderIdleGoals(); renderIdleRebirth();} else { if(!box.querySelector('#idlePublicBox')){box.innerHTML='<div id="idlePublicBox" class="idle-public"></div>'; idleLoadPublic();} } }

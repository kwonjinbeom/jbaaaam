...
(function(){
  ...
  function idleTitleList(){ return [
    {name:'이불 밖은 위험해',life:0},
    {name:'알바생',life:8000},
    {name:'프리랜서',life:50000},
    {name:'주니어 개발자',life:200000},
    {name:'시니어 개발자',life:1000000},
    {name:'팀장',life:5000000},
    {name:'스타트업 대표',life:20000000},
    {name:'투자자',life:100000000},
    {name:'건물주',life:1000000000},
    {name:'재벌',life:100000000000},
    {name:'우주 자본가',life:10000000000000}
  ]; }
  ...
  function renderIdleTitles(){ const box=$i('idleTitleBox'); if(!box)return; const life=n(idlePlayer&&idlePlayer.lifetime_money); const current=idleTitle(life); const rows=idleTitleList().map(t=>{const ok=life>=t.life; return `<div class="idle-spend-item ${ok?'done':''}"><span><b>${html(t.name)}${current===t.name?' · 현재':''}</b><small>총 번 돈 ${won(t.life)} 이상</small></span><button type="button" disabled>${ok?'획득':'잠김'}</button></div>`;}).join(''); box.innerHTML=`<div class="idle-panel"><h3>칭호 기준</h3><p class="idle-muted">칭호는 현재 보유 돈이 아니라 누적 총 번 돈 기준으로 자동 적용돼.</p>${rows}</div>`; }

...
(function(){
  ...
  async function brickLoadPublicStats(){
    try{
      const rank=await sbFetch('/brick_player_rankings?select=*&order=max_stage.desc,money.desc,updated_at.desc&limit=20');
      renderBrickStats(rank||[]);
    }catch(e){
      ...
      view.innerHTML=`
        <div class="game-card brick-shell">
          <div class="game-header-row">
            <button class="menu-btn" id="menuBtnBrick" type="button">☰</button>
  ...
  function injectBrickStyle(){
    ...
    const st=document.createElement('style'); st.id='brickRpgStyle'; st.textContent=`
      ...
      .brick-shell .game-header-row{display:flex!important;align-items:flex-start!important}.brick-shell .game-header-row>div{min-width:0;flex:1}.brick-shell .mini-btn{width:auto}
    `; document.head.appendChild(st);
  }
  function bindBrickUi(){
    ...
    $b('menuBtnBrick').onclick=openDrawer;
...
(function(){
  ...
  const IDLE_SECTION_KEY='jbaaaam_idle_section_v1';
  const IDLE_REBIRTH_NOTICE_KEY='jbaaaam_idle_rebirth_notice_v1';
  ...
  let idleReady=false,idlePlayer=null,idleUpgrades={},idleStats=null,idleTickTimer=null,idleRenderTimer=null,idleSaveTimer=null,idleEventTimer=null,idleCasinoBusy=false,idleRebirthBusy=false,idleRebirthPromptOpen=false,idleWorkHoldTimer=null,idleSuppressWorkClick=false,idleLastRender=0;
  ...
  const IDLE_UPGRADES=[
    ...
    {key:'morning_routine',cat:'life',name:'아침 루틴',max:25,base:180000,growth:1.24,clickMult:.010,incomeMult:.010,desc:'생활 리듬을 잡아 전체 효율 증가'},
    {key:'networking',cat:'side',name:'인맥 관리',max:35,base:75000,growth:1.22,income:850,incomeMult:.005,desc:'일감과 기회가 조금씩 늘어남'},
    {key:'template_shop',cat:'side',name:'템플릿 판매',max:40,base:110000,growth:1.22,income:1250,desc:'한 번 만들어두면 계속 팔림'},
    {key:'online_class',cat:'side',name:'온라인 강의',max:35,base:210000,growth:1.23,income:2800,incomeMult:.004,desc:'지식을 상품화해서 방치수익 증가'},
    {key:'notion_system',cat:'auto',name:'노션 자동정리',max:35,base:140000,growth:1.22,income:1900,desc:'작업 관리가 자동으로 굴러감'},
    {key:'sales_funnel',cat:'auto',name:'자동 영업 깔때기',max:32,base:330000,growth:1.24,income:6100,incomeMult:.005,desc:'잠자는 동안 문의가 쌓임'},
    {key:'micro_saas',cat:'auto',name:'미니 SaaS',max:30,base:820000,growth:1.25,income:16500,incomeMult:.007,desc:'작은 서비스가 구독료를 벌어옴'},
    {key:'bond',cat:'invest',name:'채권 사다리',max:45,base:170000,growth:1.21,income:1950,incomeMult:.004,desc:'안정적인 투자 수익'},
    {key:'parking_lot',cat:'invest',name:'주차장 운영',max:32,base:620000,growth:1.25,income:11500,incomeMult:.006,desc:'작지만 강한 부동산 수익'},
    {key:'franchise',cat:'invest',name:'프랜차이즈 지분',max:26,base:1800000,growth:1.27,income:42000,incomeMult:.010,desc:'후반 장기 수익원'},
    {key:'agency_team',cat:'job',name:'알바 팀장',max:35,base:68000,growth:1.22,income:780,incomeMult:.004,desc:'알바가 개인 노동에서 작은 팀으로 확장'},
    {key:'skill_book',cat:'click',name:'기술서 정독',max:35,base:38000,growth:1.21,click:420,clickMult:.006,desc:'클릭 효율과 기본 실력이 같이 증가'}
  ];
  const IDLE_BALANCE={
    typing:{base:2200,growth:1.18,click:62},focus:{base:6200,growth:1.19,clickMult:.034},critical:{base:18000,growth:1.20,crit:.018},skill_book:{base:90000,growth:1.20,click:1800,clickMult:.012},
    delivery:{base:1600,growth:1.16,income:30},warehouse:{base:5200,growth:1.17,income:95},cafe:{base:14000,growth:1.18,income:230},event_staff:{base:46000,growth:1.19,income:760},agency_team:{base:180000,growth:1.20,income:2600,incomeMult:.006},
    blog:{base:9000,growth:1.17,income:190},shorts:{base:26000,growth:1.18,income:520},design_gig:{base:85000,growth:1.19,income:1700},coding_gig:{base:160000,growth:1.19,income:3400},networking:{base:310000,growth:1.19,income:6200,incomeMult:.006},template_shop:{base:620000,growth:1.20,income:12500},online_class:{base:1500000,growth:1.20,income:36000,incomeMult:.008},
    macro:{base:35000,growth:1.18,income:850},excel_auto:{base:90000,growth:1.18,income:2300},ai_doc:{base:240000,growth:1.19,income:7200},ai_code:{base:620000,growth:1.20,income:18000},notion_system:{base:1200000,growth:1.20,income:36000},agency_bot:{base:2600000,growth:1.21,income:92000,incomeMult:.006},sales_funnel:{base:6500000,growth:1.22,income:240000,incomeMult:.010},micro_saas:{base:18000000,growth:1.23,income:820000,incomeMult:.014},
    saving:{base:45000,growth:1.17,income:900,incomeMult:.006},dividend:{base:150000,growth:1.18,income:3600,incomeMult:.008},bond:{base:380000,growth:1.19,income:9800,incomeMult:.008},etf:{base:900000,growth:1.20,income:26000,incomeMult:.010},estate_piece:{base:2800000,growth:1.21,income:95000,incomeMult:.014},parking_lot:{base:7200000,growth:1.22,income:260000,incomeMult:.016},building:{base:22000000,growth:1.23,income:980000,incomeMult:.020},franchise:{base:58000000,growth:1.24,income:2800000,incomeMult:.024},
    monitor:{base:28000,growth:1.18,clickMult:.020,incomeMult:.006},pc:{base:95000,growth:1.19,clickMult:.034,incomeMult:.010},sleep:{base:260000,growth:1.20,offline:.28,incomeMult:.012},mental:{base:620000,growth:1.20,eventLuck:.016,incomeMult:.018},morning_routine:{base:1600000,growth:1.21,clickMult:.020,incomeMult:.030}
  };
  ...
  function idleTitleList(){ return [
    ...
    {name:'루틴 장인',round:3},
    {name:'월급 탈출자',round:4},
    {name:'자동화 설계자',round:5},
    {name:'자본 굴리는 사람',round:6},
    {name:'경제적 자유인',round:7},
    {name:'인생 리세마라 고수',round:8},
    {name:'현실 조작자',round:9},
    {name:'시간 부자',round:10},
    {name:'우주 자본가',round:11}
  ]; }
  ...
  function idleLegacyPerks(){return [
    ...
    {key:'legacy_click',name:'손 빠름',max:20,cost:1,desc:'클릭 수익 +5% / Lv'},
    {key:'legacy_offline',name:'잠자는 동안 돈벌기',max:15,cost:1,desc:'오프라인 보상 +30분 / Lv'},
    {key:'legacy_discount',name:'협상왕',max:12,cost:1,desc:'일반 업그레이드 비용 -1.2% / Lv'},
    {key:'legacy_compound',name:'복리 감각',max:15,cost:2,desc:'클릭/자동수익 +2% / Lv'},
    {key:'legacy_crit',name:'손맛 기억',max:10,cost:1,desc:'크리티컬 확률 +1% / Lv'},
    {key:'legacy_event',name:'좋은 촉',max:10,cost:1,desc:'좋은 이벤트 확률 +1.5% / Lv'},
    {key:'legacy_starter',name:'재시작 자금',max:8,cost:2,desc:'환생 직후 시작 자금 +50,000원 / Lv'}
  ]; }
  function idleRebirthRequirement(){
  ...
  function deriveIdleStats(){
    ...
    const spend=idleSpendBonus(); clickMult+=spend.click; incomeMult+=spend.income; offlineHours+=spend.offline;
    const baseClick=Math.max(1,click*clickMult);
    const baseIncome=Math.max(0,income*incomeMult);
    const prestige=Number(idlePlayer&&idlePlayer.prestige_level)||0;
    const legacyClickAdd=idleLevel('legacy_click')*.05;
    const legacyAllMult=1+idleLevel('legacy_compound')*.02;
    const prestigeMult=1+prestige*.08+idleLevel('legacy_income')*.03;
    clickMult+=legacyClickAdd;
    crit+=idleLevel('legacy_crit')*.01;
    eventLuck+=idleLevel('legacy_event')*.015;
    offlineHours+=idleLevel('legacy_offline')*.5;
    const clickBeforeEvent=Math.max(1,click*clickMult*prestigeMult*legacyAllMult);
    const incomeBeforeEvent=Math.max(0,income*incomeMult*prestigeMult*legacyAllMult);
    ...
    if(until>Date.now()){
      if(ev==='awakening')activeMult*=2;
      if(ev==='burnout')activeMult*=0.7;
      if(ev==='lucky_day')activeMult*=1.45;
      if(ev==='paid_focus')activeMult*=1.35;
      if(ev==='paid_boost')activeMult*=1.75;
      if(ev==='coupon_click')eventClickMult*=1.8;
    }
    click=Math.max(1,clickBeforeEvent*eventClickMult*activeMult); income=Math.max(0,incomeBeforeEvent*activeMult);
    const bonus={
      clickLegacy:Math.max(0,clickBeforeEvent-baseClick),
      incomeLegacy:Math.max(0,incomeBeforeEvent-baseIncome),
      clickBuff:Math.max(0,click-clickBeforeEvent),
      incomeBuff:Math.max(0,income-incomeBeforeEvent)
    };
    return {click,income,crit:Math.min(.35,crit),offlineHours:Math.min(12,offlineHours),eventLuck:Math.min(.28,eventLuck),totalLv,activeMult,bonus};
  }
  ...
  async function idleSavePlayer(patch={}){ if(!idlePlayer)return; idleStats=deriveIdleStats(); const prestige=Number(patch.prestige_level??idlePlayer.prestige_level)||0; const body={...patch,title:idleTitle(prestige+1),click_power:idleStats.click,income_per_sec:idleStats.income,last_seen_at:nowIso(),updated_at:nowIso()}; const rows=await sbFetch(`/idle_players?id=eq.${encodeURIComponent(idlePlayer.id)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify(body)}); if(rows&&rows[0])idlePlayer=rows[0]; return idlePlayer; }
  ...
  function idleBlockHeldWorkKey(e){}
  function idleStopWorkHold(){ if(idleWorkHoldTimer)clearInterval(idleWorkHoldTimer); idleWorkHoldTimer=null; }
  function idleStartWorkHold(e){ if(e&&e.button!=null&&e.button!==0)return; if(e&&e.cancelable)e.preventDefault(); idleStopWorkHold(); idleSuppressWorkClick=true; idleWorkClick(); idleWorkHoldTimer=setInterval(idleWorkClick,95); }
  function idleHandleWorkClick(e){ if(idleSuppressWorkClick){ idleSuppressWorkClick=false; if(e&&e.preventDefault)e.preventDefault(); return; } idleWorkClick(); }
  function idleUpgradeBatchInfo(key,count=1){ const u=UPG[key]; if(!u)return {qty:0,total:0,nextLv:0}; const unlock=idleUnlockInfo(u); if(!unlock.ok)return {qty:0,total:0,nextLv:idleLevel(key),locked:true,reason:unlock.text}; let lv=idleLevel(key),money=n(idlePlayer&&idlePlayer.money),total=0,qty=0; const maxCount=Math.max(1,Number(count)||1); while(qty<maxCount&&lv<u.max){ const cost=idleCost(u,lv); if(money<cost)break; money-=cost; total+=cost; lv++; qty++; } return {qty,total,nextLv:lv}; }
  async function idleBuyUpgrade(key,count=1){ if(!idlePlayer){idleToast('이름을 먼저 불러와줘','error');return;} const u=UPG[key]; if(!u)return; const lv=idleLevel(key),unlock=idleUnlockInfo(u); if(!unlock.ok){idleToast(`잠김: ${unlock.text}`,'error');return;} if(lv>=u.max){idleToast('이미 최대 레벨이야');return;} const batch=idleUpgradeBatchInfo(key,count); if(!batch.qty){idleToast(`돈 부족: ${won(idleCost(u,lv))}`,'error');return;} idlePlayer.money=n(idlePlayer.money)-batch.total; idleUpgrades[key]=batch.nextLv; idleStats=deriveIdleStats(); await idleSaveUpgrade(key,batch.nextLv); await idleSavePlayer({money:n(idlePlayer.money),lifetime_money:n(idlePlayer.lifetime_money)}); idleRenderAll(); idleToast(`${u.name} Lv.${batch.nextLv}${batch.qty>1?` · ${batch.qty}개 구매`:''}`); }
  function idleClaimed(key){return Number(idleUpgrades[key]||0)>0;}
  function idleTotalUpgradeLevel(){return IDLE_UPGRADES.reduce((sum,u)=>sum+idleLevel(u.key),0);}
  function idleGoalList(){ const money=n(idlePlayer&&idlePlayer.money), life=n(idlePlayer&&idlePlayer.lifetime_money), clicks=n(idlePlayer&&idlePlayer.total_clicks), cps=idleStats?n(idleStats.income):0, totalLv=idleTotalUpgradeLevel(); return [
    {key:'goal_click_50',name:'첫 출근 50번',desc:'일하기 50회 누르기',ok:clicks>=50,reward:8000},
    {key:'goal_money_50k',name:'첫 5만원 모으기',desc:'총 번 돈 50,000원 달성',ok:life>=50000,reward:30000},
    {key:'goal_upgrade_20',name:'업그레이드 감 잡음',desc:'업그레이드 총합 Lv.20 달성',ok:totalLv>=20,reward:90000},
    {key:'goal_cps_1000',name:'잠자는 돈맛',desc:'초당 수익 1,000원/s 달성',ok:cps>=1000,reward:220000},
    {key:'goal_million',name:'첫 백만장자',desc:'총 번 돈 1,000,000원 달성',ok:life>=1000000,reward:900000},
    {key:'goal_upgrade_120',name:'시스템이 굴러간다',desc:'업그레이드 총합 Lv.120 달성',ok:totalLv>=120,reward:4500000},
    {key:'goal_cps_50000',name:'자동수익 머신',desc:'초당 수익 50,000원/s 달성',ok:cps>=50000,reward:18000000},
    {key:'goal_ten_million',name:'인생 폈다',desc:'총 번 돈 10,000,000원 달성',ok:life>=10000000,reward:35000000}
  ]; }
  async function idleClaimGoal(key){ if(!idlePlayer)return; idleStats=deriveIdleStats(); const g=idleGoalList().find(x=>x.key===key); if(!g)return; if(idleClaimed(key)){idleToast('이미 받은 목표야');return;} if(!g.ok){idleToast('아직 목표 달성 전이야','error');return;} idleUpgrades[key]=1; idlePlayer.money=n(idlePlayer.money)+g.reward; idlePlayer.lifetime_money=n(idlePlayer.lifetime_money)+g.reward; await idleSaveUpgrade(key,1); await idleSavePlayer({money:n(idlePlayer.money),lifetime_money:n(idlePlayer.lifetime_money)}); await idleLogEvent('goal',g.name,'목표 보상',g.reward); idleRenderAll(); idleToast(`${g.name} 보상 +${won(g.reward)}`); }
  function idleDailyKey(){ const d=new Date(); return `daily_${d.getFullYear()}_${String(d.getMonth()+1).padStart(2,'0')}_${String(d.getDate()).padStart(2,'0')}`; }
  function idleDailyReward(){ idleStats=deriveIdleStats(); return Math.max(5000,Math.floor(Math.min(n(idlePlayer&&idlePlayer.lifetime_money)*.024+12000,(idleStats?idleStats.income:0)*1500+12000))); }
  async function idleClaimDaily(){ if(!idlePlayer){idleToast('이름을 먼저 불러와줘','error');return;} const key=idleDailyKey(); if(idleClaimed(key)){idleToast('오늘 보상은 이미 받았어');return;} const reward=idleDailyReward(); idleUpgrades[key]=1; idlePlayer.money=n(idlePlayer.money)+reward; idlePlayer.lifetime_money=n(idlePlayer.lifetime_money)+reward; await idleSaveUpgrade(key,1); await idleSavePlayer({money:n(idlePlayer.money),lifetime_money:n(idlePlayer.lifetime_money)}); await idleLogEvent('daily','오늘의 출석','일일 보상',reward); idleRenderAll(); idleToast(`오늘의 보상 +${won(reward)}`); }
  function idleBuffList(){ const money=n(idlePlayer&&idlePlayer.money), income=idleStats?n(idleStats.income):0; return [
    {key:'paid_focus',name:'집중 카페',desc:'4분간 전체 수익 +35%',dur:240,cost:Math.max(2500,Math.floor(income*420+money*.025))},
    {key:'paid_boost',name:'마감 부스터',desc:'75초간 전체 수익 +75%',dur:75,cost:Math.max(9000,Math.floor(income*520+money*.04))},
    {key:'coupon_click',name:'각성 커피',desc:'3분간 클릭 수익 +80%',dur:180,cost:Math.max(1500,Math.floor((idleStats?n(idleStats.click):10)*160+money*.012))}
  ]; }
  async function idleBuyBuff(key){ if(!idlePlayer){idleToast('이름을 먼저 불러와줘','error');return;} idleStats=deriveIdleStats(); const b=idleBuffList().find(x=>x.key===key); if(!b)return; const until=idlePlayer.active_event_until?new Date(idlePlayer.active_event_until).getTime():0; if(until>Date.now()){idleToast('이미 진행 중인 버프/이벤트가 있어','error');return;} if(n(idlePlayer.money)<b.cost){idleToast(`돈 부족: ${won(b.cost)}`,'error');return;} idlePlayer.money=n(idlePlayer.money)-b.cost; idlePlayer.active_event_key=b.key; idlePlayer.active_event_until=new Date(Date.now()+b.dur*1000).toISOString(); await idleSavePlayer({money:n(idlePlayer.money),active_event_key:b.key,active_event_until:idlePlayer.active_event_until}); await idleLogEvent('buff',b.name,b.desc,-b.cost); idleRenderAll(); idleToast(`${b.name} 시작`); }
  ...
  function idleMaybeRandomEvent(){ if(!idlePlayer||Math.random()>.38)return; const luck=idleStats?idleStats.eventLuck:0; const events=[
    ...
    {key:'coupon_click',label:'미친 집중력',text:'45초간 클릭 수익 +80%',dur:45,good:true},
    {key:'tax_refund',label:'환급금 입금',text:'3분치 수익 즉시 획득',money:()=>Math.max(1000,(idleStats&&idleStats.income||0)*180),good:true}
  ];
  ...
  function idleCasinoInfo(){
    return {
      coin:{label:'동전 뒤집기',hint:'자산의 약 10% · 승률 48% · 적중 시 2배',desc:'가장 무난하지만 평균은 살짝 손해'},
      slot:{label:'삼단 슬롯',hint:'자산의 약 18% · 꽝/회수/대박',desc:'중간 위험, 자산이 커질수록 체감이 커짐'},
      lotto:{label:'인생역전 로또',hint:'자산의 약 30% · 승률 8% · 적중 시 10배',desc:'크게 잃거나 크게 터지는 고위험 게임'}
    };
  }
  ...
  function idleOpportunityList(){
    idleStats=deriveIdleStats();
    const money=n(idlePlayer&&idlePlayer.money);
    const income=idleStats?n(idleStats.income):0;
    return [
      {key:'urgent_gig',name:'긴급 외주 계약',kind:'safe',desc:'현재 초당수익을 기준으로 짧은 일감을 즉시 처리해 돈을 받음',cost:0,reward:Math.max(1200,Math.floor(income*75+(idleStats?idleStats.click:10)*22)),cool:35},
      {key:'used_flip',name:'중고 플립 장사',kind:'trade',desc:'자산 일부로 물건을 사서 되팔기. 손해도 있지만 카지노보다 완만함',cost:Math.max(1500,Math.floor(money*.12)),cool:45},
      {key:'micro_launch',name:'작은 사업 실험',kind:'venture',desc:'자산을 크게 걸고 작은 서비스를 출시. 실패/본전/대박이 갈림',cost:Math.max(7000,Math.floor(money*.22)),cool:90}
    ];
  }
  function idleOppCooldownKey(key){return `jbaaaam_idle_opp_cd_${key}_${idlePlayer&&idlePlayer.id||'anon'}`;}
  function idleOppCooldownLeft(key){ const t=Number(localStorage.getItem(idleOppCooldownKey(key))||0); return Math.max(0,Math.ceil((t-Date.now())/1000)); }
  function idleSetOppCooldown(key,sec){ localStorage.setItem(idleOppCooldownKey(key),String(Date.now()+sec*1000)); }
  async function idleRunOpportunity(key){
    if(!idlePlayer){idleToast('이름을 먼저 불러와줘','error');return;}
    const o=idleOpportunityList().find(x=>x.key===key); if(!o)return;
    const left=idleOppCooldownLeft(key); if(left>0){idleToast(`${left}초 뒤에 다시 가능해`,'error');return;}
    const money=n(idlePlayer.money); if(o.cost>0&&money<o.cost){idleToast(`돈 부족: ${won(o.cost)}`,'error');return;}
    let delta=0,text='';
    if(o.kind==='safe'){
      delta=o.reward; text='계약 완료';
    }else if(o.kind==='trade'){
      const r=Math.random(); let mult=0;
      if(r<.12)mult=.62; else if(r<.34)mult=.88; else if(r<.72)mult=1.18; else if(r<.93)mult=1.42; else mult=2.15;
      delta=Math.floor(o.cost*(mult-1)); text=mult<1?'매입가보다 싸게 처분':mult<1.3?'소소한 차익':mult<1.8?'괜찮은 마진':'희귀템 대박';
    }else if(o.kind==='venture'){
      const r=Math.random(); let mult=0;
      if(r<.28)mult=.58; else if(r<.56)mult=.94; else if(r<.82)mult=1.28; else if(r<.96)mult=1.85; else mult=3.4;
      delta=Math.floor(o.cost*(mult-1)); text=mult<.8?'출시 실패':mult<1?'거의 본전':mult<1.5?'초기 고객 확보':mult<2.4?'입소문 성공':'작은 대박';
    }
    idleSetOppCooldown(key,o.cool);
    idlePlayer.money=Math.max(0,money+delta);
    if(delta>0)idlePlayer.lifetime_money=n(idlePlayer.lifetime_money)+delta;
    await idleSavePlayer({money:n(idlePlayer.money),lifetime_money:n(idlePlayer.lifetime_money)});
    await idleLogEvent('opportunity_'+key,o.name,text,delta);
    idleRenderAll();
    idleToast(`${o.name}: ${text} · ${delta>=0?'+':''}${won(delta)}`,delta<0?'error':'ok');
  }
  function renderIdleOpportunities(){
    const box=$i('idleOpportunityBox'); if(!box)return;
    const ops=idleOpportunityList();
    box.innerHTML=`<div class="idle-panel idle-opportunity-panel"><h3>판 벌리기</h3><p class="idle-muted">단순 업그레이드 말고, 현재 자산과 수익 상태를 활용해서 한 번씩 돈을 벌어보는 별도 콘텐츠야. 카지노보다 성장에 가깝지만 손실 가능성은 있어.</p><div class="idle-opportunity-grid">${ops.map(o=>{const left=idleOppCooldownLeft(o.key),money=n(idlePlayer&&idlePlayer.money),disabled=!idlePlayer||left>0||(o.cost>0&&money<o.cost); return `<button type="button" data-opp="${o.key}" ${disabled?'disabled':''}><b>${html(o.name)}</b><small>${html(o.desc)}</small><small>${o.cost?`투입 ${won(o.cost)}`:`예상 보수 ${won(o.reward)}`} · ${left>0?left+'초 대기':'가능'}</small></button>`;}).join('')}</div></div>`;
    box.querySelectorAll('[data-opp]').forEach(b=>b.onclick=()=>idleRunOpportunity(b.dataset.opp));
  }
  ...
  function renderIdleCasino(){
    ...
    box.innerHTML=`<div class="idle-panel idle-casino-panel"><h3>카지노</h3><p class="idle-muted">카지노는 업그레이드가 아니라 별도 도박 콘텐츠야. 배팅금은 현재 자산에 비례해서 커지고, 평균적으로는 조금 손해라서 성장 핵심은 여전히 업그레이드가 맞아.</p><div class="idle-casino-grid">${Object.entries(info).map(([k,v])=>`<button type="button" data-casino="${k}" ${!idlePlayer||money<100?'disabled':''}><b>${html(v.label)}</b><small>${html(v.hint)}</small><small>예상 배팅 ${idlePlayer?won(idleCasinoBet(k,money)):'-'}</small></button>`).join('')}</div></div>`;
    box.querySelectorAll('[data-casino]').forEach(b=>b.onclick=()=>idleCasino(b.dataset.casino));
  }
  function renderIdleGoals(){ const box=$i('idleGoalsBox'); if(!box)return; idleStats=deriveIdleStats(); const dailyKey=idleDailyKey(); const dailyDone=idlePlayer&&idleClaimed(dailyKey); const goals=idlePlayer?idleGoalList():[]; box.innerHTML=`<div class="idle-panel idle-goal-panel"><h3>오늘의 보너스</h3><div class="idle-daily-row"><span><b>출석 보상</b><small>하루 한 번, 성장 속도를 크게 흔들지 않는 보너스</small></span><button type="button" id="idleDailyBtn" ${!idlePlayer||dailyDone?'disabled':''}>${dailyDone?'수령완료':won(idleDailyReward())}</button></div></div><div class="idle-panel"><h3>인생 목표</h3>${goals.length?goals.map(g=>{const done=idleClaimed(g.key); return `<div class="idle-goal ${done?'done':g.ok?'can':''}"><span><b>${html(g.name)}</b><small>${html(g.desc)} · 보상 ${won(g.reward)}</small></span><button type="button" data-goal="${g.key}" ${done||!g.ok?'disabled':''}>${done?'완료':g.ok?'받기':'진행중'}</button></div>`;}).join(''):'<p class="idle-muted">이름을 먼저 불러와줘.</p>'}</div>`; const daily=$i('idleDailyBtn'); if(daily)daily.onclick=idleClaimDaily; box.querySelectorAll('[data-goal]').forEach(b=>b.onclick=()=>idleClaimGoal(b.dataset.goal)); }
  function renderIdleTitles(){ const box=$i('idleTitleBox'); if(!box)return; const round=idleCurrentRound(); const current=idleTitle(round); const rows=idleTitleList().map(t=>{const ok=round>=t.round; return `<div class="idle-spend-item ${ok?'done':''}"><span><b>${html(t.name)}${current===t.name?' · 현재':''}</b><small>인생 ${t.round}회차 달성</small></span><button type="button" disabled>${ok?'획득':'잠김'}</button></div>`;}).join(''); box.innerHTML=`<div class="idle-panel"><h3>칭호 기준</h3><p class="idle-muted">칭호는 돈이 아니라 인생 회차 기준으로 자동 적용돼.</p>${rows}</div>`; }
  function renderIdleBuffShop(){ const box=$i('idleBuffBox'); if(!box)return; idleStats=deriveIdleStats(); const until=idlePlayer&&idlePlayer.active_event_until?new Date(idlePlayer.active_event_until).getTime():0; const busy=until>Date.now(); const buffs=idleBuffList(); box.innerHTML=`<div class="idle-panel idle-buff-panel"><h3>소모품 버프</h3><p class="idle-muted">영구 업그레이드가 아니라 잠깐 쓰는 부스터야. 기존 밸런스를 깨지 않게 비용은 현재 성장에 맞춰 자동 조정돼.</p><div class="idle-buff-grid">${buffs.map(b=>`<button type="button" data-buff="${b.key}" ${!idlePlayer||busy||n(idlePlayer.money)<b.cost?'disabled':''}><b>${html(b.name)}</b><small>${html(b.desc)}</small><small>비용 ${won(b.cost)}</small></button>`).join('')}</div></div>`; box.querySelectorAll('[data-buff]').forEach(b=>b.onclick=()=>idleBuyBuff(b.dataset.buff)); }

  function idleSection(){ const v=localStorage.getItem(IDLE_SECTION_KEY)||'spend'; return ['spend','opportunity','casino','goals','rank','titles'].includes(v)?v:'spend'; }
...
획득 포인트: ${info.points}`); if(!ok)return; idleRebirthBusy=true; const btn=$i('idleRebirthBtn'); if(btn){btn.disabled=true;btn.textContent='처리 중...';} try{ const rebirthPlayerId=idlePlayer.id; const beforePrestige=n(idlePlayer.prestige_level); const keep={}; Object.keys(idleUpgrades).forEach(k=>{ if(k.startsWith('legacy_')||k.startsWith('spend_'))keep[k]=idleUpgrades[k]; }); for(const k of Object.keys(idleUpgrades)){ if(!(k.startsWith('legacy_')||k.startsWith('spend_'))){ try{ await sbFetch(`/idle_upgrades?player_id=eq.${encodeURIComponent(rebirthPlayerId)}&upgrade_key=eq.${encodeURIComponent(k)}`,{method:'DELETE',headers:{Prefer:'return=minimal'}}); }catch(e){} } }
    keep.legacy_points=Number(keep.legacy_points||0)+info.points; idleUpgrades=keep; await idleSaveUpgrade('legacy_points',keep.legacy_points); const starterMoney=idleLevel('legacy_starter')*50000; idlePlayer.money=starterMoney; idlePlayer.lifetime_money=starterMoney; idlePlayer.total_clicks=0; idlePlayer.prestige_level=beforePrestige+1; idlePlayer.active_event_key=null; idlePlayer.active_event_until=null; await idleSavePlayer({money:starterMoney,lifetime_money:starterMoney,total_clicks:0,prestige_level:beforePrestige+1,active_event_key:null,active_event_until:null}); await idleLoadUpgrades(); idleRenderAll(); idleToast(`인생 ${beforePrestige+2}회차 시작 · 포인트 +${info.points}${starterMoney?` · 시작 자금 +${won(starterMoney)}`:''}`); }catch(e){ console.warn(e); idleToast('회차 처리 중 오류가 났어. 새로고침 후 다시 확인해줘.','error'); } finally { idleRebirthBusy=false; idleRenderAll(); } }
  ...
  function renderIdleRebirth(){
    ...
    box.querySelectorAll('[data-legacy]').forEach(b=>b.onclick=()=>idleBuyLegacyPerk(b.dataset.legacy));
  }
  function renderIdleExtra(){ const box=$i('idleExtraBox'); if(!box)return; const sec=idleSection(); document.querySelectorAll('.idle-section-tab').forEach(b=>b.classList.toggle('active',b.dataset.section===sec)); if(sec==='spend'){box.innerHTML='<div id="idleSpendBox"></div>'; renderIdleSpend();} else if(sec==='opportunity'){box.innerHTML='<div id="idleBuffBox"></div><div id="idleOpportunityBox"></div>'; renderIdleBuffShop(); renderIdleOpportunities();} else if(sec==='casino'){box.innerHTML='<div id="idleCasinoBox"></div>'; renderIdleCasino();} else if(sec==='goals'){box.innerHTML='<div id="idleRebirthBox"></div><div id="idleGoalsBox"></div>'; renderIdleRebirth(); renderIdleGoals();} else if(sec==='titles'){box.innerHTML='<div id="idleTitleBox"></div>'; renderIdleTitles();} else { if(!box.querySelector('#idlePublicBox')){box.innerHTML='<div id="idlePublicBox" class="idle-public"></div>'; idleLoadPublic();} } }

  async function idleLoadPublic(){ const box=$i('idlePublicBox'); if(!box)return; try{ const rank=await sbFetch('/idle_player_rankings?select=*&order=prestige_level.desc,lifetime_money.desc&limit=20'); const me=idlePlayer&&idlePlayer.name; box.innerHTML=`<div class="idle-panel"><h3>전체 회차 랭킹</h3><p class="idle-muted">회차를 먼저 보고, 같은 회차 안에서는 총 번 돈 순서로 정렬해.</p>${(rank||[]).length?(rank||[]).map((r,i)=>{const round=Number(r.prestige_level||0)+1; return `<div class="idle-rank ${me===r.name?'me':''}"><b>${i+1}</b><span>${html(r.name)}<small>${round}회차 · ${html(idleTitle(round))} · 업그레이드 Lv.${Number(r.total_upgrade_level||0)}</small></span><strong>${compact(r.lifetime_money)}원</strong></div>`;}).join(''):'<p class="idle-muted">아직 랭킹 없음</p>'}</div>`; }catch(e){ box.innerHTML='<p class="idle-muted">랭킹을 불러오지 못했어. SQL 실행 여부를 확인해줘.</p>'; } }
  function idleStatBonusHtml(kind){ const b=idleStats&&idleStats.bonus||{}, legacy=n(b[kind+'Legacy']), buff=n(b[kind+'Buff']), suffix=kind==='income'?'/s':''; const parts=[]; if(legacy>=1)parts.push(`<em class="idle-bonus-legacy">+${won(legacy)}${suffix}</em>`); if(buff>=1)parts.push(`<em class="idle-bonus-buff">+${won(buff)}${suffix}</em>`); return parts.length?`<small class="idle-stat-bonuses">${parts.join('')}</small>`:''; }
  function idleStatValueHtml(value,kind,suffix=''){ return `${won(value)}${suffix}${idleStatBonusHtml(kind)}`; }
  function idleRenderAll(){ idleStats=deriveIdleStats(); if(!$i('idlePlayerName'))return; const p=idlePlayer; $i('idlePlayerName').textContent=p?p.name:'-'; $i('idleTitleText').textContent=p?idleTitle(idleCurrentRound(p)):'-'; const rt=$i('idleRoundText'); if(rt)rt.textContent=p?`${idleCurrentRound(p)}회차`:'1회차'; $i('idleMoneyText').textContent=p?won(p.money):'0원'; $i('idleLifeText').textContent=p?won(p.lifetime_money):'0원'; $i('idleCpsText').innerHTML=p?idleStatValueHtml(idleStats.income,'income','/s'):'0원/s'; $i('idleClickText').innerHTML=p?idleStatValueHtml(idleStats.click,'click'):'0원'; $i('idleOfflineText').textContent=p?`최대 ${idleStats.offlineHours.toFixed(1)}시간`:'-'; const ev=$i('idleEventText'); if(ev){ const until=p&&p.active_event_until?new Date(p.active_event_until).getTime():0; ev.textContent=until>Date.now()?`${eventName(p.active_event_key)} ${Math.ceil((until-Date.now())/1000)}초`:'이벤트 없음'; }
    renderIdleQuickPanel(); renderIdleUpgrades(); renderIdleExtra(); idleMaybePromptRebirth(); }
  function eventName(k){return {awakening:'각성',burnout:'번아웃',lucky_day:'운수 좋은 날',paid_focus:'집중 카페',paid_boost:'마감 부스터',coupon_click:'미친 집중력'}[k]||'이벤트';}
  function renderIdleUpgrades(){ const list=$i('idleUpgradeList'); if(!list)return; const savedTab=localStorage.getItem(IDLE_TAB_KEY)||'click'; const tab=IDLE_TABS.some(([k])=>k===savedTab)?savedTab:'click'; if(tab!==savedTab)localStorage.setItem(IDLE_TAB_KEY,tab);
    const arr=IDLE_UPGRADES.filter(u=>u.cat===tab); list.innerHTML=arr.map(u=>{ const lv=idleLevel(u.key),cost=idleCost(u,lv),max=lv>=u.max,unlock=idleUnlockInfo(u),batch=idleUpgradeBatchInfo(u.key,10),can=idlePlayer&&unlock.ok&&!max&&n(idlePlayer.money)>=cost,can10=idlePlayer&&unlock.ok&&!max&&batch.qty>0; return `<div class="idle-upgrade ${can?'can':''} ${unlock.ok?'':'locked'}"><div><b>${html(u.name)} <em>Lv.${lv}/${u.max}</em></b><small>${html(u.desc)}</small><small>${unlock.ok?`다음 비용 ${max?'MAX':won(cost)}${!max&&batch.qty?` · x${batch.qty} ${won(batch.total)}`:''}`:`잠김 · ${html(unlock.text)}`}</small></div><div class="idle-upgrade-actions"><button type="button" data-up="${u.key}" ${!can?'disabled':''}>${max?'MAX':unlock.ok?'구매':'잠김'}</button><button type="button" data-up10="${u.key}" ${!can10?'disabled':''}>x10</button></div></div>`; }).join(''); list.querySelectorAll('[data-up]').forEach(b=>b.onclick=()=>idleBuyUpgrade(b.dataset.up,1)); list.querySelectorAll('[data-up10]').forEach(b=>b.onclick=()=>idleBuyUpgrade(b.dataset.up10,10)); renderIdleCasino(); }
  ...
  function injectIdleStyle(){ if($i('idleStyle'))return; const st=document.createElement('style'); st.id='idleStyle'; st.textContent=`
    .idle-shell .game-header-row{display:flex!important;justify-content:space-between;gap:8px;align-items:flex-start}.idle-shell .game-header-row>div{min-width:0;flex:1}.idle-shell .game-header-row h2{margin:0;font-size:22px}.idle-shell .game-header-row p{margin:5px 0 0;color:#6b7280;font-size:13px;font-weight:800}.idle-shell .menu-btn{flex:0 0 42px}.idle-shell #idleRefreshBtn{flex:0 0 auto;width:auto;margin-top:0;white-space:nowrap;padding:0 10px}
    .idle-card .app-icon{background:linear-gradient(135deg,#fef3c7,#fb923c)!important}.idle-page{display:none}.idle-page.active{display:block}.idle-shell{display:grid;gap:12px}.idle-login{display:flex;gap:8px;align-items:center}.idle-login input{flex:1}.idle-status-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.idle-status-grid>div{background:rgba(15,23,42,.06);border:1px solid rgba(15,23,42,.08);border-radius:14px;padding:10px}.idle-status-grid span{display:block;font-size:12px;color:#64748b}.idle-status-grid strong{display:block;font-size:17px}.idle-stat-bonuses{display:flex;flex-wrap:wrap;gap:4px;margin-top:3px;font-size:11px;line-height:1.2}.idle-stat-bonuses em{font-style:normal;font-weight:1000}.idle-bonus-buff{color:#dc2626}.idle-bonus-legacy{color:#16a34a}.idle-work-btn{width:100%;border:0;border-radius:22px;padding:20px;font-weight:1000;font-size:22px;background:linear-gradient(135deg,#fde68a,#fb923c);color:#111827;box-shadow:0 10px 22px rgba(251,146,60,.25)}.idle-tabs{display:flex;gap:6px;overflow-x:auto;padding-bottom:2px}.idle-tab{white-space:nowrap;border:1px solid rgba(15,23,42,.1);background:#fff;border-radius:999px;padding:8px 11px;font-weight:900}.idle-tab.active{background:#111827;color:#fff}.idle-upgrade-list{display:grid;gap:8px}.idle-upgrade{display:flex;justify-content:space-between;gap:10px;align-items:center;border:1px solid rgba(15,23,42,.1);border-radius:14px;padding:10px;background:#fff}.idle-upgrade.can{border-color:#fb923c;box-shadow:0 0 0 2px rgba(251,146,60,.12)}.idle-upgrade.locked{opacity:.62;background:#f8fafc}.idle-upgrade b{display:block}.idle-upgrade em{font-style:normal;color:#f97316}.idle-upgrade small{display:block;color:#64748b;font-size:12px}.idle-upgrade-actions{display:flex;gap:6px;flex-shrink:0}.idle-upgrade button,.idle-casino-grid button{border:0;border-radius:12px;padding:10px 12px;background:#111827;color:#fff;font-weight:900}.idle-upgrade button:disabled{background:#cbd5e1}.idle-casino-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.idle-casino-panel{background:linear-gradient(135deg,#fff7ed,#fff)!important}.idle-casino-grid button{min-height:102px;background:linear-gradient(135deg,#111827,#7c2d12)}.idle-casino-grid button:disabled{background:#cbd5e1;color:#64748b}.idle-casino-grid small{display:block;color:#fed7aa;font-size:11px;margin-top:6px}.idle-casino-grid button:disabled small{color:#64748b}.idle-public{display:grid;grid-template-columns:1fr 1fr;gap:10px}.idle-panel{border:1px solid rgba(15,23,42,.1);border-radius:16px;background:#fff;padding:10px}.idle-panel h3{margin:0 0 8px}.idle-rank,.idle-log{display:flex;justify-content:space-between;gap:8px;align-items:center;border-top:1px solid rgba(15,23,42,.06);padding:7px 0}.idle-rank:first-of-type,.idle-log:first-of-type{border-top:0}.idle-rank.me{background:#fef3c7;border-radius:10px;padding-left:6px;padding-right:6px}.idle-rank span,.idle-log span{display:flex;flex-direction:column}.idle-rank small,.idle-log small,.idle-muted{color:#64748b;font-size:12px}.idle-log strong.good{color:#16a34a}.idle-log strong.bad{color:#dc2626}.idle-goal,.idle-daily-row{display:flex;justify-content:space-between;gap:10px;align-items:center;border-top:1px solid rgba(15,23,42,.06);padding:9px 0}.idle-goal:first-of-type,.idle-daily-row:first-of-type{border-top:0}.idle-goal span,.idle-daily-row span{display:flex;flex-direction:column}.idle-goal.can{background:#fff7ed;border-radius:12px;padding-left:8px;padding-right:8px}.idle-goal.done{opacity:.62}.idle-goal button,.idle-daily-row button{border:0;border-radius:12px;padding:9px 12px;background:#111827;color:#fff;font-weight:900}.idle-goal button:disabled,.idle-daily-row button:disabled{background:#cbd5e1;color:#64748b}.idle-goal-panel{background:linear-gradient(135deg,#f8fafc,#fff)!important}.idle-buff-panel{background:linear-gradient(135deg,#eff6ff,#fff)!important}.idle-buff-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.idle-buff-grid button{border:0;border-radius:14px;padding:12px;background:linear-gradient(135deg,#1d4ed8,#111827);color:#fff;font-weight:900;text-align:left}.idle-buff-grid button:disabled{background:#cbd5e1;color:#64748b}.idle-buff-grid small{display:block;color:#dbeafe;font-size:11px;margin-top:5px}.idle-buff-grid button:disabled small{color:#64748b}.idle-opportunity-panel{background:linear-gradient(135deg,#f0fdf4,#fff)!important}.idle-opportunity-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.idle-opportunity-grid button{border:0;border-radius:14px;padding:12px;background:linear-gradient(135deg,#166534,#111827);color:#fff;font-weight:900;text-align:left}.idle-opportunity-grid button:disabled{background:#cbd5e1;color:#64748b}.idle-opportunity-grid small{display:block;color:#dcfce7;font-size:11px;margin-top:5px}.idle-opportunity-grid button:disabled small{color:#64748b}.idle-main-layout{display:grid;grid-template-columns:minmax(280px,.85fr) minmax(320px,1.15fr);gap:12px}.idle-core-panel{display:grid;gap:10px}.idle-growth-panel{display:grid;gap:8px}.idle-panel-title{display:flex;justify-content:space-between;gap:10px;align-items:end}.idle-panel-title h3{margin:0}.idle-panel-title small{color:#64748b}.idle-quick-box{display:grid;grid-template-columns:1fr 1fr;gap:8px}.idle-quick-card{border:1px solid rgba(15,23,42,.1);border-radius:16px;background:#fff;padding:10px;display:grid;gap:6px}.idle-quick-card small{color:#64748b;font-size:12px}.idle-quick-card button{border:0;border-radius:12px;padding:9px;background:#111827;color:#fff;font-weight:900}.idle-quick-card button:disabled{background:#cbd5e1;color:#64748b}.idle-section-tabs{display:flex;gap:6px;overflow-x:auto;padding:2px 0}.idle-section-tab{white-space:nowrap;border:1px solid rgba(15,23,42,.1);background:#fff;border-radius:999px;padding:9px 12px;font-weight:1000}.idle-section-tab.active{background:#fb923c;color:#111827;border-color:#fb923c}.idle-extra-box{display:grid;gap:10px}.idle-spend-score,.idle-rebirth-status{display:flex;flex-wrap:wrap;gap:8px;align-items:center}.idle-spend-score span,.idle-rebirth-status span,.idle-rebirth-status b,.idle-rebirth-status small{border-radius:999px;background:#f8fafc;border:1px solid rgba(15,23,42,.08);padding:6px 9px;font-size:12px}.idle-rebirth-status span.ok{background:#dcfce7;border-color:#86efac;color:#166534;font-weight:900}.idle-rebirth-status b{background:#ede9fe;color:#5b21b6;font-weight:1000}.idle-spend-head{background:linear-gradient(135deg,#fefce8,#fff)!important}.idle-spend-item{display:flex;justify-content:space-between;gap:10px;align-items:center;border-top:1px solid rgba(15,23,42,.06);padding:9px 0}.idle-spend-item:first-of-type{border-top:0}.idle-spend-item span{display:flex;flex-direction:column}.idle-spend-item small{color:#64748b;font-size:12px}.idle-spend-item.can{background:#fff7ed;border-radius:12px;padding-left:8px;padding-right:8px}.idle-spend-item.done{opacity:.72}.idle-spend-item button,.idle-rebirth-panel button{border:0;border-radius:12px;padding:9px 12px;background:#111827;color:#fff;font-weight:900}.idle-spend-item button:disabled,.idle-rebirth-panel button:disabled{background:#cbd5e1;color:#64748b}.idle-rebirth-panel{background:linear-gradient(135deg,#f5f3ff,#fff)!important}.idle-rebirth-panel>button{width:100%;margin-top:10px;padding:12px}.idle-public{grid-template-columns:1fr}.idle-extra-box .idle-public{display:block}@media(max-width:720px){.idle-main-layout{grid-template-columns:1fr}.idle-public{grid-template-columns:1fr}.idle-casino-grid,.idle-buff-grid,.idle-opportunity-grid{grid-template-columns:1fr}.idle-status-grid{grid-template-columns:1fr 1fr}.idle-quick-box{grid-template-columns:1fr}.idle-work-btn{padding:18px}}
  `; document.head.appendChild(st); }
  function ensureIdleUi(){
    ...
    if(!$i('idleView')){ const view=document.createElement('section'); view.id='idleView'; view.className='view-page idle-page'; view.innerHTML=`
      <div class="game-card idle-shell">
        <div class="game-header-row"><button class="menu-btn" id="menuBtnIdle" type="button">☰</button><div><h2>백수 키우기</h2><p>일하기로 시작해서 알바, 부업, 자동화, 투자, 카지노까지 굴리는 방치형 RPG.</p></div><button id="idleRefreshBtn" class="mini-btn" type="button">랭킹 새로고침</button></div>
        ...
        <div class="idle-section-tabs">
          <button class="idle-section-tab" data-section="spend" type="button">돈 쓰기</button>
          <button class="idle-section-tab" data-section="opportunity" type="button">판 벌리기</button>
          <button class="idle-section-tab" data-section="casino" type="button">카지노</button>
          <button class="idle-section-tab" data-section="goals" type="button">목표/2회차</button>
          <button class="idle-section-tab" data-section="rank" type="button">랭킹</button>
          <button class="idle-section-tab" data-section="titles" type="button">칭호</button>
        </div>
        <div id="idleExtraBox" class="idle-extra-box"></div>
      </div>`; main.appendChild(view); }
    $i('idleNameInput').value=currentName(); $i('menuBtnIdle').onclick=openDrawer; $i('idleLoadBtn').onclick=idleLoadByName; $i('idleSaveBtn').onclick=()=>idleFlushSave().then(()=>idleToast('저장 완료')); const workBtn=$i('idleWorkBtn'); workBtn.onclick=idleHandleWorkClick; workBtn.onkeydown=idleBlockHeldWorkKey; workBtn.onpointerdown=idleStartWorkHold; workBtn.onpointerup=idleStopWorkHold; workBtn.onpointerleave=idleStopWorkHold; workBtn.onpointercancel=idleStopWorkHold; $i('idleRefreshBtn').onclick=idleLoadPublic; const activeIdleTab=IDLE_TABS.some(([k])=>k===(localStorage.getItem(IDLE_TAB_KEY)||'click'))?(localStorage.getItem(IDLE_TAB_KEY)||'click'):'click'; document.querySelectorAll('.idle-tab').forEach(b=>{b.onclick=()=>setIdleTab(b.dataset.tab); b.classList.toggle('active',b.dataset.tab===activeIdleTab);}); document.querySelectorAll('.idle-section-tab').forEach(b=>{b.onclick=()=>setIdleSection(b.dataset.section); b.classList.toggle('active',b.dataset.section===idleSection());}); renderIdleUpgrades(); renderIdleExtra();
  }

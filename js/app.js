...
(function(){
  ...
  let omokReady=false, omokCanvas=null, omokCtx=null, omokRoom=null, omokMoves=[], omokPlayerName='', omokStone='', omokRealtimeClient=null, omokChannel=null, omokPollTimer=null, omokLastHash='', omokPlaceBusy=false;
  ...
  function omokInstallDom(){ if(omokSafe('omokView'))return; omokInstallCss(); const app=document.querySelector('.app')||document.body; const grid=document.querySelector('.app-grid'); if(grid&&!omokSafe('homeOmokCard')){ const card=document.createElement('button'); card.type='button'; card.id='homeOmokCard'; card.className='app-card'; card.innerHTML='<div class="app-icon">⚫</div><div><div class="app-name">오목</div><div class="app-desc">실시간 1:1 대전</div></div>'; card.onclick=()=>setMainView('omok'); grid.appendChild(card); }
    ...
    const view=document.createElement('section'); view.id='omokView'; view.className='view-page omok-page'; view.innerHTML=`
      ...
      <div class="omok-panel"><div class="section-title-row"><div class="section-title">최근 전적 이력</div><button id="omokHistoryRefreshBtn" class="tiny-btn" type="button">갱신</button></div><div class="omok-history" id="omokRecentHistory"><div class="omok-empty">아직 전적 이력이 없어.</div></div></div>`;
  ...
  async function omokSetRoom(room){ omokRoom=room; if(omokSafe('omokRoomInput'))omokSafe('omokRoomInput').value=room.room_code||''; localStorage.setItem(OMOK_ROOM_KEY,room.room_code||''); await omokLoadMoves(); await omokLoadStatsAndHistory(false); omokRender(); await omokSubscribe(); omokStartPolling(); }
  ...
  async function omokLoadStatsAndHistory(manual=false){
    ...
    if(!name){
      omokSetText('omokWins','0'); omokSetText('omokLosses','0'); omokSetText('omokWinRate','0%'); omokSetText('omokStatsMeta','이름을 입력하면 개인 전적을 불러와.');
      const h=omokSafe('omokRecentHistory'); if(h)h.innerHTML='<div class="omok-empty">이름을 입력하면 최근 전적 이력을 불러와.</div>';
      return;
    }
    try{
      ...
      const url=`/omok_recent_matches?select=*&or=(black_name.eq.${encodeURIComponent(name)},white_name.eq.${encodeURIComponent(name)})&order=finished_at.desc&limit=8`;
      const rows=await sbFetch(url);
      omokRenderHistory(Array.isArray(rows)?rows:[],name);
  ...
  function omokRenderHistory(rows,name){
    ...
    h.innerHTML=rows.map(r=>{
      const myWin=String(r.winner_name||'')===String(name||'');
      const result=myWin?'승리':'패배';
      const cls=myWin?'omok-result-win':'omok-result-lose';
      const opp=String(r.black_name||'')===String(name||'')?(r.white_name||'-'):(r.black_name||'-');
      const moves=Number(r.move_count)||0;
      const dt=omokFormatDate(r.finished_at||r.updated_at||r.created_at);
      return `<div class="omok-history-row"><div><b class="${cls}">${result}</b> · 상대 ${omokHtml(opp)}<div class="omok-history-sub">${omokHtml(r.room_code||'')} · ${moves}수 · ${dt}</div></div><span>${omokStoneText(r.winner_stone)}</span></div>`;
    }).join('');
  }
